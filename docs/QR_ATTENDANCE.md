...
2. A **short-lived, branch-bound gym QR session/token** as the reusable attendance point at the gym.

The QR/token carries **no member identity**. The member's identity is resolved server-side from the authenticated session; the QR session only proves "this check-in is happening at this branch right now, against a valid short-lived token." This prevents a static QR from being a permanent key to any member's data.

## 2. Concepts

- **QRSession** — a server-issued, cryptographically random, short-lived token bound to a **branch** (a gym attendance point), with an `expires_at` and optional `revoked_at`. It is **reusable**: multiple members may check in against the same token during its validity. Only the **hash** of the token is stored.
- **QR payload** — the opaque token string (or a URL containing it). Contains no member ID. Cannot be guessed (high-entropy random).
- **CheckIn** — the attendance record binding the **authenticated member**, the branch, the QR session, and a timestamp.

## 3. Check-in flow (authenticated member flow)

```
1. Staff member displays a short-lived QRSession token for the branch
   (server issues a branch-bound QR token; only the hash is stored).
2. Member authenticates in the member app / portal (server resolves identity).
3. Member scans/submits the current QR token for the branch.
4. Server validates:
     - token exists, not revoked, not expired
     - token's branch matches the check-in branch
     - the authenticated member has an active membership at that branch
...
- **Tenancy**: the QR session and check-in are organization-scoped; the staff user must have `attendance:record` and be assigned to that branch (`UserBranch`) — a restricted staff member can only issue/scan sessions for branches they are assigned to.
- **Active branch gate**: QR session creation and scans are rejected for an **INACTIVE** branch (`branch_inactive`) — only ACTIVE branches can issue attendance points.
- **Membership validation**: a check-in requires an `active` membership status for the member **in that branch** (with configured grace period). See `PRODUCT_REQUIREMENTS.md` FR-QR-05 / FR-MS-04.
- **Duplicate window**: a configurable window (default e.g. 5 minutes) rejects repeated check-ins of the same member at the same branch.
- **Token handling**:
...
- **Wrong branch**: token is bound to a branch; using it at another branch's reader is rejected.
...
- **QRSession (branch-bound, reusable)**: `token_hash` (unique), `organization_id`, `branch_id` (the attendance point), `expires_at`, `revoked_at` (nullable), `created_by_user_id`, `created_at`. There is **no member binding** on the session.
- **CheckIn**: `branch_id`, `member_id`, `qr_session_id`, `checked_in_at`, `source` (`qr-session`).

## 8. Compatibility with the tenant model

QRSession and CheckIn both carry `organization_id` and are reachable only through the tenant-scoped data layer, exactly like members, memberships, and payments. This keeps attendance on the same isolation boundary as the rest of the product — and additionally **branch-scoped** for restricted staff, so attendance screens, requests, and exports only ever show the staff member's assigned branches (see `AUTHORIZATION.md` §5.5).
